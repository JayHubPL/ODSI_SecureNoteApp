from flask import Blueprint, redirect, url_for, render_template, flash, Response, request
from flask_login import login_required
from .utils import get_validated_note, check_password_strength, generate_flash_msg
...
def change_encryption_status(note_id):
    ...
    if note.is_encrypted:
        ...
        Note.query.filter_by(id=note.id).update(dict(is_encrypted=False, content=content_decrypted))
        db.session.commit()
    else:
...
def add_password(note_id):
    ...
    if note.is_encrypted:
        ...
        Note.query.filter_by(id=note.id).update(dict(is_encrypted=True, content=encrypted_content, password=pwhash))
        db.session.commit()
    return redirect(url_for('main.profile'))
